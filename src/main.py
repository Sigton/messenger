...
class MainPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.entry = tk.Text(self, height=2, width=52, bg=DARK_BACKGROUND_COLOUR, font=TEXTBOX_FONT)
        ...
        self.send_button.grid(row=2, column=2, sticky="ew", ipady=2, pady=3, padx=5)

        self.display = tk.Text(self, height=18, width=63, state="disabled", bg=TEXTBOX_COLOUR, font=TEXTBOX_FONT)
        self.display.grid(row=1, column=0, columnspan=4, sticky="nsew", ipadx=13, ipady=5, padx=5)

        self.online_users = tk.Text(self, height=3, width=40, state="disabled", bg=TEXTBOX_COLOUR, font=TEXTBOX_FONT)
        self.online_users.grid(row=0, column=1, columnspan=3, sticky="ew", pady=5, padx=5)

        self.server_name = tk.Label(self, text="Woodpecker", font=HEADING_FONT, height=2, width=10)
        self.server_name.grid(row=0, column=0)

        self.randomise_colour = tk.Button(self, text="\u2744", width=4, command=self.random_colour, bg=BUTTON_COLOUR, font=MEDIUM_FONT, activebackground=BUTTON_ACTIVE_COLOUR)
        self.randomise_colour.grid(row=2, column=3, sticky="ew", ipady=2, pady=3, padx=5)
    ...
    def random_colour(self):
        ...
        self.name_colour = random.randint(0,len(self.controller.name_colours)-1)
        self.randomise_colour.config(fg=self.controller.name_colours[self.name_colour])
    ...
    def refresh(self):
        ...
        for message in messages:
            ...
            if message[3]:
                self.display.insert(tk.END, " {}".format(message[1]), ("username{}".format(int(message[4]))))
                ...
                try:
                    ...
                    self.display.insert(tk.END, " {}\n".format(username), ("username{}".format(int(message[4])),))
                except:
                    self.display.insert(tk.END, " {}\n".format(message[2]))
